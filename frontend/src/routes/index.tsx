        ...
        <div className="p-6 flex-1 overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-between items-center group">
              ...
              <span className={`text-xs font-mono px-2 py-1 rounded-sm border border-white/5 ${activeUser.status === 'online' ? 'text-emerald-400 bg-emerald-400/10' : activeUser.status === 'busy' ? 'text-amber-400 bg-amber-400/10' : 'text-zinc-400 bg-zinc-800'}`}>
          ...
          <div className="grid grid-cols-7 gap-1">
            {[...Array(35)].map((_, i) => (
              <div key={i} className={`aspect-square rounded-xs ${Math.random() > 0.7 ? 'bg-indigo-500/40 shadow-[0_0_5px_rgba(99,102,241,0.2)]' : 'bg-zinc-800/50'}`} />
            ))}
          </div>
